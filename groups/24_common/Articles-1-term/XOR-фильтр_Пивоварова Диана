...
**XOR-фильтр** — это компактная структура данных, которая почти не занимает места в памяти и молниеносно проверяет, точно ли какого-то элемента нет в нашем исходном наборе данных.
...
У нас есть три функции (как три подсказки), которые для любого элемента (например, ника) говорят, в какие три ячейки массива нужно посмотреть.
...
- Выбираем три хеш-функции: h1(x), h2(x), h3(x). Они для любого ника будут выдавать три случайных, но всегда одинаковых для этого ника числа. Это будут номера ящиков в нашей коробке.
- Выбираем функцию отпечатка (fingerprint): f(x). Она превращает ник в небольшое число (скажем, 32 бита) — его «цифровой код» или «отпечаток».
...
- «Распутываем» граф. Берём такое «висячее» ребро (элемент x). Мы можем однозначно вычислить, какое число должно быть в свободной вершине (ячейке), чтобы уравнение для x выполнилось. Допустим, две другие ячейки уже заполнены (или пока нули). 
...
*O(1)*
Неважно, у вас 1000 элементов или 1 000 000 в чёрном списке — проверка займёт одно и то же, фиксированное число операций (всего несколько вычислений и доступов к памяти).
